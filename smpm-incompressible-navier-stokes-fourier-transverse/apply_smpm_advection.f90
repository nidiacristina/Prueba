subroutine apply_smpm_advection( Aq, q, ux_in, uy_in, uz_in )
! Computes the advective operator on q in skew-symmetric form.
  ...
  real(kind=dp), dimension(1:rpk, 1:nsuby)              :: div_uq
  real(kind=dp), dimension(1:rpk, 1:nsuby)              :: uxq, uyq, uzq
  ...
  uyq = uy_in * q
  uzq = uz_in * q
  call compute_3D_divergence( uxq, uyq, uzq, div_uq )

  ! Compute the advection operator in skew-symmetric form.
  Aq = -0.5_dp * (ux_in * q_x + uy_in * q_y + uz_in * q_z + div_uq)
  ...
  do ll = 1, nsuby
     ...
     do ii = 1, numA_per_rank - 1
        do kk = 1, nsubz * n
           ...
           uDOTn(left) = - 1 * uDOTn(left) ! The left side of an interface is
                                           ! the right side of a subdomain.
...
end subroutine apply_smpm_advection
