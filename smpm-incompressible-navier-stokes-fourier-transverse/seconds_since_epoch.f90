...
! Computes the fractional seconds since midnight on January 1st, 1970 in UTC.
  ...
  real(kind=dp), intent(out) :: timestamp
  ...
  ! go back to UTC.
  timestamp = (todays_day - epoch_day) * 86400.0_dp + &
              current_time_array(5) * 3600.0_dp + &
              (current_time_array(6) - current_time_array(4)) * 60.0_dp + &
              current_time_array(7) + &
              current_time_array(8) / 1000.0_dp
return
