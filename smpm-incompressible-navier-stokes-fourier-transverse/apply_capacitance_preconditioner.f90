subroutine apply_capacitance_preconditioner( Cx, x )
! Returns Cx = x + B(A\(Ex))
!
...
! Cx distributed across all MPI processes.
  ...
  use woodbury_matrices, only: aCk, dimCk, numA_per_rank, preC, preC_pivot, s, &
                               zCk
  ...
  do ii = 1, numA_per_rank
     ...
     if ( mod( block_number, 2 ) == 0 ) then
        ...
        if ( block_number == nsubx ) then
           ...
           call DGETRS( 'N', 2*s, 1, preC(1:2*s, 1:2*s, ii), 2*s, preC_pivot(1:2*s, ii), &
                        iix(1:2*s), 2*s, info )
           ...
           ! Perform the division.
           call DGETRS( 'N', 4*s, 1, preC(1:4*s, 1:4*s, ii), 4*s, preC_pivot(1:4*s, ii), &
                        iix(1:4*s), 4*s, info )
...
end subroutine apply_capacitance_preconditioner
