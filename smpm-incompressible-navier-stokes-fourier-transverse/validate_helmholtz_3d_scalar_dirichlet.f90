subroutine validate_helmholtz_3d_scalar_dirichlet( tolerance, success_flag )
! Validates the scalar solver with dirichlet bcs using a real-valued GMRES 
...
!  Greg Thomsen & Gustavo Rivera
  ...
  if (nsuby > 1) then
     ...
     rho_ana = sin( kk * cx ) * sin( ll * cy ) * sin( mm * cz )

  else

     ! Dirichlet boundary conditions.
     rho_ana = sin( kk * cx ) * sin( mm * cz )
  ...
  if (nsuby > 1) then
     ! Dirichlet boundary conditions.
     rhs_ana = -(nu_d * dt / g0) * (kk**2  + ll**2  + mm**2 ) * sin( kk * cx ) * sin( ll * cy ) * sin( mm * cz ) &
               -( sin( kk * cx ) * sin( ll * cy ) * sin( mm * cz ) )
  else
     ! Dirichlet boundary conditions.
     rhs_ana = -(nu_d * dt / g0) * (kk**2.0_dp + mm**2.0_dp) * sin( kk * cx ) * sin( mm * cz ) - ( sin( kk * cx ) * sin( mm * cz ) )
  ...
  call MPI_ALLREDUCE( l2_sol, l2_sol_max, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_WORLD, ierr )
   ...
   if (rank .eq. root) then
      open(unit=65,file='HSD_error.txt')
...
end subroutine validate_helmholtz_3d_scalar_dirichlet
