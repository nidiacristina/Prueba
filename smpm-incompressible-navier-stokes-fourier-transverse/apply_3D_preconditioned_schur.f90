subroutine apply_3D_preconditioned_schur( Sx, x_in )
! Returns Sx = x + B(A\(Ex))
!
...
! Sx distributed across all MPI processes.

  use constants, only:         nky, nprocs, nsubx, rank
  use precision, only:         dp
  use woodbury_matrices, only: dimCk, numA_per_rank, S_poisson, s

  implicit none

  real(kind=dp), dimension(1:dimCk,1:nky), intent(out) :: Sx
  real(kind=dp), dimension(1:dimCk,1:nky), intent(in)  :: x_in
  real(kind=dp), dimension(1:dimCk,1:nky)              :: x
  ...
  Sx = 0.0_dp
  call apply_3D_schur_preconditioner( Sx, x_in )
  x  = Sx
  ...
  do kk = 1,nky
     ...
     do ii = 1, numA_per_rank
        ...
        !                 nrows, x(a:z, kk), 1, 0.0_dp, iiCx, 1 )
...
end subroutine apply_3D_preconditioned_schur
