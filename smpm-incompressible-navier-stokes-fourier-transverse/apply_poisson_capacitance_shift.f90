subroutine apply_poisson_capacitance_shift( CTx, x )
...
! problem, and B the off-diagonal boundary block of the Poisson problem.
  ...
  do kk = 1, numA_per_rank
     ...
     counter = 1
     do ii = a, z
        CTx(ii) = DDOT( size( iix(rowA:rowZ), 1 ), iix(rowA:rowZ), 1, C_poisson_block(CrowA:CrowZ, offset + counter, kk), 1 )
        counter = counter + 1
     enddo
...
end subroutine apply_poisson_capacitance_shift
