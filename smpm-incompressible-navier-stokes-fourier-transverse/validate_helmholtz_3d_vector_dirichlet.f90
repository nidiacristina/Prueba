subroutine validate_helmholtz_3d_vector_dirichlet( tolerance, success_flag )
...
!  Greg Thomsen & Gustavo Rivera 
  ...
  if (nsuby > 1) then
   ...
   l2_ux_rhs = pmaxval( reshape( abs( rhs_ana_ux - rhs_num_ux ), (/rpk * nsuby/) ) )
   ...
   l2_uz_rhs = pmaxval( reshape( abs( rhs_ana_uz - rhs_num_uz ), (/rpk * nsuby/) ) )
  ...
  call MPI_ALLREDUCE( l2_uz_sol, l2_uz_sol_max, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_WORLD, ierr )
   ...
   if (rank .eq. root) then
      open(unit=65,file='HVD_error.txt')
...
end subroutine validate_helmholtz_3d_vector_dirichlet
